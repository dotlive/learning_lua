...
IF(CMAKE_C_COMPILER_ID MATCHES MSVC)
    SET(MAGE_COMPILER_NAME "vc")
    SET(MAGE_COMPILER_MSVC TRUE)
    IF(MSVC_VERSION GREATER 1900)
        SET(MAGE_COMPILER_VERSION "141")
    ELSE()
    ...
    IF(MSVC_VERSION GREATER 1900)
        ...
        IF(MSVC_VERSION GREATER 1910)
            IF(MAGE_PLATFORM_WINDOWS_STORE OR (MAGE_ARCH_NAME STREQUAL "arm64"))
        ...
        ENDIF()
    ENDIF()
    SET(CMAKE_CXX_FLAGS_RELEASE "${CMAKE_CXX_FLAGS_RELEASE} /DMAGE_SHIP /fp:fast /Ob2 /GL /Qpar")
    ...
    IF((MSVC_VERSION GREATER 1912) AND (MAGE_ARCH_NAME STREQUAL "x64"))
    ...
    IF(MAGE_ARCH_NAME MATCHES "arm")
        ADD_DEFINITIONS(-D_ARM_WINAPI_PARTITION_DESKTOP_SDK_AVAILABLE=1)

        IF(MAGE_PLATFORM_WINDOWS_DESKTOP)
            FOREACH(flag_var
    ...
    IF(MAGE_PLATFORM_WINDOWS_STORE)
        FOREACH(flag_var
...
ELSE()
    IF(CMAKE_C_COMPILER_ID MATCHES Clang)
        SET(MAGE_COMPILER_NAME "clang")
        SET(MAGE_COMPILER_CLANG TRUE)
        IF(MSVC)
            SET(MAGE_COMPILER_CLANGC2 TRUE)
        ENDIF()
    ELSEIF(MINGW)
        SET(MAGE_COMPILER_NAME "mgw")
        SET(MAGE_COMPILER_GCC TRUE)
    ELSE()
        SET(MAGE_COMPILER_NAME "gcc")
        SET(MAGE_COMPILER_GCC TRUE)
    ENDIF()
    IF((NOT MSVC) AND MAGE_PLATFORM_WINDOWS)
        ADD_DEFINITIONS(-D_WIN32_WINNT=0x0601 -DWINVER=_WIN32_WINNT)
    ENDIF()

    IF(MAGE_COMPILER_CLANG)
    ...
    FOREACH(flag_var
        ...
        ENDIF()
        IF(MAGE_COMPILER_CLANG AND (MAGE_PLATFORM_DARWIN OR MAGE_PLATFORM_IOS))
    ...
    IF(MAGE_ARCH_NAME STREQUAL "x86")
        ...
        IF(NOT MSVC)
            ...
            ENDIF()
        ENDIF()
    ELSEIF((MAGE_ARCH_NAME STREQUAL "x64") OR (MAGE_ARCH_NAME STREQUAL "x86_64"))
        FOREACH(flag_var
            CMAKE_C_FLAGS CMAKE_CXX_FLAGS)
        ...
        ENDIF()
    ENDIF()
    IF((NOT MSVC) AND (NOT MAGE_HOST_PLATFORM_DARWIN))
        FOREACH(flag_var
...
IF(MAGE_COMPILER_MSVC)
    SET(RTTI_FLAG "/GR")
    SET(NO_RTTI_FLAG "/GR-")
ELSE()
    SET(RTTI_FLAG "-frtti")
    IF(MAGE_COMPILER_CLANGC2)
        SET(NO_RTTI_FLAG "")
    ELSE()
...
IF(MSVC)
    ...
    ENDFUNCTION()
ENDIF()
IF(MAGE_PLATFORM_DARWIN OR MAGE_PLATFORM_IOS)
...
FUNCTION(CREATE_PROJECT_USERFILE PROJECTNAME TARGETNAME)
    ...
    ELSEIF(MAGE_PLATFORM_DARWIN OR MAGE_PLATFORM_IOS)
